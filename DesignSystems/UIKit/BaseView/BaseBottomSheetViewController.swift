...
///
/// Subclasses override `buildContent()` and add their UI into `contentView`
/// (pinned to the container's leading/trailing/bottom edges, below the handle
/// when `showsHandle` is enabled). Close the sheet via `dismissSheet(feedback:completion:)`.
class BaseBottomSheetViewController: UIViewController {
    struct Configuration {
        ...
        /// Off-screen offset (below the container's bottom edge) the sheet starts at before animating in.
    ...
    /// Subclasses add their own subviews/constraints here.
    ...
    /// Override to add subviews/constraints into `contentView`.
    func buildContent() {}

    /// Override to customize what happens when the dimming backdrop is tapped.
    /// Default forwards to `dismissSheet`.
    func didTapDimming() {
