...
final class RouteTimelineView: UIView {
    ...
    private let stack: UIStackView = {
        ...
        stack.axis = .vertical
        // Zero spacing is what keeps the line unbroken between rows — each row
        // draws its own half of every joint, above and below its ring.
        stack.spacing = 0
        return stack
    }()
    ...
    /// Each row owns two line segments: one from its top edge up to the ring, one
    /// from the ring down to its bottom edge. A single segment per row would leave
    /// a gap across every row boundary, because the rings sit at the rows' centres
    /// rather than at their edges. The first row hides its upper segment and the
    /// last its lower, so the line starts and ends on a ring.
