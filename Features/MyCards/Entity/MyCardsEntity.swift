...
enum MyCardsViewState: Equatable {
    case loading
    /// An empty array is the empty state — the "Add new card" row renders either way.
    case loaded([CardItem])
    case failed(String)
}

/// Raw form output. Lives for exactly one submit call and is never persisted:
/// `cvv` is read on the way to the gateway and dropped, never written to Firestore.
struct NewCardInput {
    let number: String
