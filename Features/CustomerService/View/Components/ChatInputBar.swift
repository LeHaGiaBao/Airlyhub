...
///
/// A `UITextView` rather than a `UITextField` because a support message is often several
/// lines; the view grows with its content up to `Metric.maxTextHeight` and scrolls after
/// that, so the bar can never eat the whole screen.
final class ChatInputBar: UIView {
    ...
    /// Decorative: iOS gives no API to switch the keyboard to its emoji plane, so tapping
    /// this just brings the keyboard up rather than pretending to do something it can't.
    ...
    private let sendButton: UIButton = {
        ...
        // Hidden until there is something to send — matches the resting state in the
        // design, where the bar shows only the emoji and attachment affordances.
    ...
    /// Blocks a second tap while an attachment uploads. Swaps the send glyph for a
    /// spinner in place so the bar's width doesn't jump.
...
private extension ChatInputBar {
    ...
    /// The bar is outlined and white while in use, flat grey while idle — the two states
    /// in the design.
...
extension ChatInputBar: UITextViewDelegate {
    func textViewDidChange(_ textView: UITextView) {
        ...
        // Grow with the content, then hand over to scrolling once the cap is reached.
