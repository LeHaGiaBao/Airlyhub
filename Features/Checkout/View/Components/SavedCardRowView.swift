...
///
/// Deliberately *not* `CardCell`, even though the two look the same. That type is a
/// `UITableViewCell`: its height comes from the table that owns it and its
/// `contentView` is constrained with no position of its own, so dropped into a
/// `UIStackView` it collapses to nothing. The shared parts are the formatting and
/// the artwork — `CardFormatter.masked` and `CardBrand.logo` — which this reuses;
/// only the container is its own.
final class SavedCardRowView: UIView {
    ...
    /// Carries the card's id so the screen can resolve a tap without holding an
    /// index that a reload may have invalidated — same contract as
    /// `TourCardView.onToggleFavorite`.
    ...
    func configure(with card: CardModel, isSelected: Bool) {
        ...
        // The badge artwork carries its own background, so the placeholder chrome
        // is only drawn for `.unknown` — matching `CardCell.configure`.
    ...
    private func setupUI() {
        ...
        // A fixed height rather than one derived from the labels: every row shows
        // the same two lines, and a stack of equal rows reads as a list.
