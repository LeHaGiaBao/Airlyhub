...
enum Validation {
    ...
    /// Optional — passes if nil/empty, fails only if present but malformed
    ...
    static func validDateOfBirth(_ value: String?) -> ValidationResult {
        ...
        let minDate = calendar.date(from: minComps)!
        let maxDate = calendar.date(from: maxComps)!
 
        if date < minDate || date > maxDate {
            let msg = NSLocalizedString("validation_invalid_dob_range", comment: "Date must be between 01/01/1900 and 01/01/\(currentYear - 16)")
